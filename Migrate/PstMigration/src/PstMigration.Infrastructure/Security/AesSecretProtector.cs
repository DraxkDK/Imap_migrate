...
namespace PstMigration.Infrastructure.Security;

/// <summary>
/// AES-GCM encryption for secrets at rest. The master key comes from configuration
/// (<c>PstMigration:MasterKey</c>, base64 of 32 bytes) and must be identical on the
/// Api and Portal (they share the DB). Generate one with:
...
public sealed class AesSecretProtector : ISecretProtector
{
    ...
    {
        var b64 = config["PstMigration:MasterKey"];
        if (string.IsNullOrWhiteSpace(b64))
            throw new InvalidOperationException("PstMigration:MasterKey is not configured (base64 of 32 bytes).");
        _key = Convert.FromBase64String(b64);
        if (_key.Length != 32)
            throw new InvalidOperationException("PstMigration:MasterKey must decode to exactly 32 bytes.");
    }
